...
class SlotAttentionModel(nn.Module):
    ...
    def forward(self, x):
        ...
        recons = out[:, :, :num_channels, :, :] # torch.Size([64, 7, 3, 128, 128])
        masks = out[:, :, -1:, :, :] # torch.Size([64, 7, 1, 128, 128])
        # Normalize alpha masks over slots.
        masks = F.softmax(masks, dim=1)
        recon_combined = torch.sum(recons * masks, dim=1) # torch.Size([64, 3, 128, 128])
        return recon_combined, recons, masks, slots # slots: shape=torch.Size([448, 64, 1, 1])
