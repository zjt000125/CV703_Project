        ...
        if trainer.logger:
            with torch.no_grad():
                ...
                trainer.logger.experiment.log(
                    {stage + "/images": [wandb.Image(images)]}, commit=True # TODO False
                )
...
def visualize(image, true_mask, recon, rgb_dec_attns, pred_default_mask, rgb_default_attns, N=8):
    _, _, H, W = image.shape  # 320, 320
    ...
    #                                     alpha=.5,
    #                                     colors = colors) for idx in range(image.shape[0])])/255.) # torch.Size([8, 3, 320, 320])
    ...
    _, true_mask_unique = torch.unique(true_mask,return_inverse=True) # torch.Size([8, 320, 320])
    ...
    image = image[:N].expand(-1, 3, H, W).unsqueeze(dim=1).cpu() # torch.Size([8, 1, 3, 320, 320])
    rgb_default_attns = rgb_default_attns[:N].expand(-1, -1, 3, H, W).cpu() # torch.Size([8, 7, 3, 320, 320])
    rgb_dec_attns = rgb_dec_attns[:N].expand(-1, -1, 3, H, W).cpu() # torch.Size([8, 7, 3, 320, 320])
 
    rgb_true_mask = rgb_true_mask.unsqueeze(dim=1).cpu() # torch.Size([8, 1, 3, 320, 320])
    rgb_pred_default_mask = rgb_pred_default_mask.unsqueeze(dim=1).cpu() # torch.Size([8, 1, 3, 320, 320])
    # rgb_pred_dec_mask = rgb_pred_dec_mask.unsqueeze(dim=1).cpu() # torch.Size([8, 1, 3, 320, 320])
    recon = recon.unsqueeze(dim=1)

    return torch.cat((image, rgb_true_mask, recon, rgb_dec_attns, rgb_pred_default_mask, rgb_default_attns), dim=1).view(-1, 3, H, W) #TODO: change rgb_pred_dec_mask to recon from slot attention